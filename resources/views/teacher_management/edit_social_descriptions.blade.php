...
@section('title', "Kelola Deskripsi Nilai Sosial Kelas " . $room_term->room->name . " Semester $room_term->even_odd")
    ...
    <table class="table table-striped table-responsive-xl table-sm">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td> <textarea data-id="{{ $report->id }}" data-prev-value="{{ $report->social_attitude_description }}" class="description form-control" rows="2">{{ $report->social_attitude_description }}</textarea></td>
            </tr>
            @endforeach
        </tbody>
    </table>
...
<script>
    ...
    $(document).ready(() => {
        ...
        $('button#btn-update').click(() => {
            ...
            $.ajax({
                method: 'POST',
                url: '{{ route('teacher.management.social_description', $room_term->id) }}',
                data: {
                    _token: '{{ csrf_token() }}',
                    data: changed
                },
